...
Portfolio built with Three.js and vanilla HTML/CSS/JS, bundled via Vite. The scene renders interactive bubbles, panels for featured work, and loads structured content from `site-data.json`.
...
- Vite (dev/build)
- Three.js (ESM)
- Vanilla HTML/CSS/JS (`src/legacy-main.js`, `src/style.css`)

## Project Structure
- `index.html` — Root HTML with Vite module entry.
- `src/main.js` — Vite entry; wires globals, loads content, bootstraps the legacy scene.
...
Build locally or in CI and publish `dist/` (e.g., deploy to `gh-pages` or set Pages to the built output). The root HTML’s `/src/main.js` is for development; production should serve the built `dist` bundle.

## Editing Content
- Structured data: edit `site-data.json` (profile links, stats, skills, experience, featured projects, bubble projects, education).
- Page copy/layout: edit `index.html` (hero, headings, lede text).
